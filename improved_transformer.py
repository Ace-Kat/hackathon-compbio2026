"""
Improved Transformer Model for Chromatin State Prediction
Run: python improved_transformer.py
"""
...
class Config:
    ...
    num_heads = 8
    num_transformer_layers = 8   # was 6
    ff_dim = 1024                # was 512
    dropout = 0.05               # was 0.1
    
    # CNN parameters - BIGGER
    cnn_channels = [128, 256, 256]  # was [64, 128, 128]
    cnn_kernels = [15, 9, 5]        # was [9, 5, 3]
...
def train_fold(fold, train_idx, val_idx, train_sequences, train_labels, config, device):
    ...
    torch.save(best_model_state, f'transformer_model_fold{fold}.pt')
...
def main():
    ...
    # Create zip
    import zipfile
    with zipfile.ZipFile('submission_transformer.zip', 'w', zipfile.ZIP_DEFLATED) as zf:
        zf.write(output_file)
    print("Created submission_transformer.zip")
