"""
🏆 MEGA ENSEMBLE - 9 Models
Combines: 5 champion folds + 4 transformer folds = 9 models
"""
...
# ============================================
# CHAMPION MODEL (embed_dim=256, 8 layers)
# ============================================
...
# ============================================
# ORIGINAL TRANSFORMER (embed_dim=128, 6 layers)
# ============================================
...
def load_all_models(device):
    ...
    # Load 5 Champion models (bigger model)
    print("\n📦 Loading Champion models...")
    champion_config = ChampionConfig()
    for i in range(5):
        path = f'champion_model_fold{i}.pt'
        try:
            model = ChromatinStatePredictor(champion_config).to(device)
    ...
    champion_config = ChampionConfig()
    for i in range(5):
        path = f'champion_model2_fold{i}.pt'
        try:
            model = ChromatinStatePredictor(champion_config).to(device)
            model.load_state_dict(torch.load(path, map_location=device))
            model.eval()
    ...
    champion_config = ChampionConfig()
    for i in range(5):
        path = f'champion_model3_fold{i}.pt'
        try:
            model = ChromatinStatePredictor(champion_config).to(device)
            model.load_state_dict(torch.load(path, map_location=device))
            model.eval()
    ...
    # Load 4 Original Transformer models (smaller model)
    print("\n📦 Loading Original Transformer models...")
    original_config = OriginalConfig()
    for i in range(4):
        path = f'transformer_model_fold{i}.pt'
        try:
            model = ChromatinStatePredictor(original_config).to(device)
            model.load_state_dict(torch.load(path, map_location=device))
            model.eval()
            models.append(model)
            print(f"  ✅ Loaded {path}")
...
def predict_with_tta(models, test_sequences, device, batch_size=256):
    ...
    for model_idx, model in enumerate(models):
        ...
        with torch.no_grad():
            for seq_orig, seq_rc in tqdm(test_loader, desc=f'Model {model_idx+1}'):
    ...
    # Ensemble: average all probabilities
...
if __name__ == '__main__':
    ...
    if len(models) == 0:
        print("❌ No models loaded! Check your .pt files.")
        exit(1)
    ...
    # Save predictions as predictions.csv (required name)
